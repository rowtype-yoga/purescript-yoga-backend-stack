...
console.log("🐳 Starting Postgres test service...");
...
try {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      const pgService = services.find(s => s.Service === "postgres-test");

      if (pgService && pgService.Health === "healthy") {
  ...
  if (attempts >= maxAttempts) {
    console.error("❌ Postgres failed to become healthy");
    process.exit(1);
  }

  // Run tests
  console.log("🧪 Running tests...\n");
  await $`bunx spago test -p yoga-postgres`;
  ...
  console.log("\n🛑 Stopping Postgres test service...");
